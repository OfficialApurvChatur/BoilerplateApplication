...
import AdminBranchSectionDeleteComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/fAdminLandingComponent/hAdminBranchSectionComponent/eDeleteComponent";
...
const AdminBranchSectionDeletePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = adminBranchSectionAPIEndpoint.useAdminBranchSectionRetrieveAPIQuery({ params: { _id: id } });
  const [ deleteAPITrigger, deleteAPIResponse ] = adminBranchSectionAPIEndpoint.useAdminBranchSectionDeleteAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.adminBranchSection, "Delete") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AdminBranchSectionDeletePage */}
...
export default AdminBranchSectionDeletePage;
