...
import AdminCounterUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/fAdminLandingComponent/fAdminCounterComponent/dUpdateComponent";
...
const AdminCounterUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = adminCounterAPIEndpoint.useAdminCounterRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = adminCounterAPIEndpoint.useAdminCounterUpdateAPIMutation();
  ...
  useSocketEventHook(socket, {
    [`ADMINCOUNTER_RETRIEVED:${id}`]: () => apiCall.retrieveAPIResponse.refetch()
  })  
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.adminCounter, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AdminCounterUpdatePage */}
      <AdminCounterUpdateComponent 
...
export default AdminCounterUpdatePage;
