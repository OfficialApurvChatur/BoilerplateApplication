...
import VideoStorageDeleteComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/dFileStorageComponent/bVideoStorageComponent/eDeleteComponent";
...
const VideoStorageDeletePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = videoStorageAPIEndpoint.useVideoStorageRetrieveAPIQuery({ params: { _id: id } });
  const [ deleteAPITrigger, deleteAPIResponse ] = videoStorageAPIEndpoint.useVideoStorageDeleteAPIMutation();
  ...
  useSocketEventHook(socket, {
    [`VIDEOSTORAGE_RETRIEVED:${id}`]: () => apiCall.retrieveAPIResponse.refetch()
  })
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.videoStorage, "Delete") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* VideoStorageDeletePage */}
...
export default VideoStorageDeletePage;
