...
import BranchGroupDeleteComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/gFrontendLandingComponent/iBranchGroupComponent/eDeleteComponent";
...
const BranchGroupDeletePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = branchGroupAPIEndpoint.useBranchGroupRetrieveAPIQuery({ params: { _id: id } });
  const [ deleteAPITrigger, deleteAPIResponse ] = branchGroupAPIEndpoint.useBranchGroupDeleteAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.branchGroup, "Delete") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* BranchGroupDeletePage */}
...
export default BranchGroupDeletePage;
