...
import AudioStorageDeleteComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/dFileStorageComponent/cAudioStorageComponent/eDeleteComponent";
...
const AudioStorageDeletePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = audioStorageAPIEndpoint.useAudioStorageRetrieveAPIQuery({ params: { _id: id } });
  const [ deleteAPITrigger, deleteAPIResponse ] = audioStorageAPIEndpoint.useAudioStorageDeleteAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.audioStorage, "Delete") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AudioStorageDeletePage */}
...
export default AudioStorageDeletePage;
