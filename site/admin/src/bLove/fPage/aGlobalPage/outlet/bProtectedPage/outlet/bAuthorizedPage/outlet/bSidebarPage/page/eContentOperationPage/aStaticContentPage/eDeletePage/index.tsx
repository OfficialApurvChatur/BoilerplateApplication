...
import StaticContentDeleteComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/eContentOperationComponent/aStaticContentComponent/eDeleteComponent";
...
const StaticContentDeletePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = staticContentAPIEndpoint.useStaticContentRetrieveAPIQuery({ params: { _id: id } });
  const [ deleteAPITrigger, deleteAPIResponse ] = staticContentAPIEndpoint.useStaticContentDeleteAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.staticContent, "Delete") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* StaticContentDeletePage */}
...
export default StaticContentDeletePage;
