...
const aboutCompanyValidation = {
  ...
  create: () => [
    ...
    body("aTitle")
      ...
      .custom(async value => {
        const retrieve = await AboutCompanyModel.findOne({aTitle: value});
  ...
  retrieve: () => [
    param("id")
      ...
      .custom(async value => {
        const retrieve = await AboutCompanyModel.findById(value);
        if (!retrieve) throw new ErrorUtility("About Company Not Found", 404)
        return true;
      }),
  ],
