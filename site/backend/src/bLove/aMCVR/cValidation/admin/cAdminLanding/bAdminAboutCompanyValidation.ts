...
const adminAboutCompanyValidation = {
  ...
  create: () => [
    ...
    body("aTitle")
      ...
      .custom(async value => {
        const retrieve = await AdminAboutCompanyModel.findOne({aTitle: value});
  ...
  retrieve: () => [
    param("id")
      ...
      .custom(async value => {
        const retrieve = await AdminAboutCompanyModel.findById(value);
        if (!retrieve) throw new ErrorUtility("Admin About Company Not Found", 404)
        return true;
      }),
  ],
