...
const adminHeroValidation = {
  ...
  create: () => [
    ...
    body("aTitle")
      ...
      .custom(async value => {
        const retrieve = await AdminHeroModel.findOne({aTitle: value});
  ...
  retrieve: () => [
    param("id")
      ...
      .custom(async value => {
        const retrieve = await AdminHeroModel.findById(value);
        if (!retrieve) throw new ErrorUtility("Admin Hero Not Found", 404)
        return true;
      }),
  ],
