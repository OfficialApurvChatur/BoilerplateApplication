...
import { AdminContactFormModel } from "../../../aModel/admin/cAdminLanding/cAdminContactFormModel";
...
const adminContactFormValidation = {
  ...
  create: () => [
    ...
    body("aTitle")
      ...
      .custom(async value => {
        const retrieve = await AdminContactFormModel.findOne({aTitle: value});
  ...
  retrieve: () => [
    param("id")
      ...
      .custom(async value => {
        const retrieve = await AdminContactFormModel.findById(value);
        if (!retrieve) throw new ErrorUtility("Admin Contact Form Not Found", 404)
        return true;
      }),
  ],
