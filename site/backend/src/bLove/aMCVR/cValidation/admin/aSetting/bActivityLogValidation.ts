...
import { ActivityLogModel } from "../../../aModel/admin/aSetting/bActivityLogModel";
...
const activityLogValidation = {
  ...
  create: () => [
    ...
    body("aTitle")
      ...
      .custom(async value => {
        const retrieve = await ActivityLogModel.findOne({aTitle: value});
  ...
  retrieve: () => [
    param("id")
      ...
      .custom(async value => {
        const retrieve = await ActivityLogModel.findById(value);
        if (!retrieve) throw new ErrorUtility("Activity Log Not Found", 404)
        return true;
      }),
  ],
