...
import { APILogModel } from "../../../aModel/admin/aSetting/cAPILogModel";
...
const apiLogValidation = {
  ...
  create: () => [
    ...
    body("aTitle")
      ...
      .custom(async value => {
        const retrieve = await APILogModel.findOne({aTitle: value});
  ...
  retrieve: () => [
    param("id")
      ...
      .custom(async value => {
        const retrieve = await APILogModel.findById(value);
        if (!retrieve) throw new ErrorUtility("API Log Not Found", 404)
        return true;
      }),
  ],
