...
const adminBranchSectionPageController = (Model= "AdminBranchSectionPageModel", Label= "AdminBranchSectionPageModel") => ({
  // Retrieve Controller
  retrieve: catchAsyncMiddleware(
    async (request: express.Request, response: express.Response, next: express.NextFunction) => {
      ...
      cacheCreateMiddleware({ 
        key: cacheVariable.adminBranchSectionPageModel.retrieve({ 
          Label, 
          request 
        }), 
        data: responseData 
...
export default adminBranchSectionPageController;
