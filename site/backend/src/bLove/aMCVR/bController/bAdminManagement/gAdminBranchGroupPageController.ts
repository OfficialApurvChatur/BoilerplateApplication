...
const adminBranchGroupPageController = (Model= "AdminBranchGroupPageModel", Label= "AdminBranchGroupPageModel") => ({
  // Retrieve Controller
  retrieve: catchAsyncMiddleware(
    async (request: express.Request, response: express.Response, next: express.NextFunction) => {
      ...
      cacheCreateMiddleware({ 
        key: cacheVariable.adminBranchGroupPageModel.retrieve({ 
          Label, 
          request 
        }), 
        data: responseData 
...
export default adminBranchGroupPageController;
