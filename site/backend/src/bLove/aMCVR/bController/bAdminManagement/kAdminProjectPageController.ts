...
const adminProjectPageController = (Model= "AdminProjectPageModel", Label= "AdminProjectPageModel") => ({
  // Retrieve Controller
  retrieve: catchAsyncMiddleware(
    async (request: express.Request, response: express.Response, next: express.NextFunction) => {
      ...
      cacheCreateMiddleware({ 
        key: cacheVariable.adminProjectPageModel.retrieve({ 
          Label, 
          request 
        }), 
        data: responseData 
...
export default adminProjectPageController;
