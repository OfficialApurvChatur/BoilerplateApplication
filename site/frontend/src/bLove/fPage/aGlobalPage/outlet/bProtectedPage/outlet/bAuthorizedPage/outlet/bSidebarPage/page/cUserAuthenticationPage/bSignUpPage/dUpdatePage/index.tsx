...
import SignUpUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/cUserAuthenticationComponent/bSignUpComponent/dUpdateComponent";
...
const SignUpUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = signUpAPIEndpoint.useSignUpRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = signUpAPIEndpoint.useSignUpUpdateAPIMutation();
  ...
  useSocketEventHook(socket, {
    [`SIGNUP_RETRIEVED:${id}`]: () => apiCall.retrieveAPIResponse.refetch()
  })  
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.signUp, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* SignUpUpdatePage */}
