...
import AudioStorageUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/dFileStorageComponent/cAudioStorageComponent/dUpdateComponent";
...
const AudioStorageUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = audioStorageAPIEndpoint.useAudioStorageRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = audioStorageAPIEndpoint.useAudioStorageUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.audioStorage, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AudioStorageUpdatePage */}
...
export default AudioStorageUpdatePage;
