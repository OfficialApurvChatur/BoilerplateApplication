...
import CounterUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/gFrontendLandingComponent/fCounterComponent/dUpdateComponent";
...
const CounterUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = counterAPIEndpoint.useCounterRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = counterAPIEndpoint.useCounterUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.counter, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* CounterUpdatePage */}
