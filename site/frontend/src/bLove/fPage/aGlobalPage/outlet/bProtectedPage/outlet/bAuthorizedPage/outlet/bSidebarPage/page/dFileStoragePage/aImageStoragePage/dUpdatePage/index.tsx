...
import ImageStorageUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/dFileStorageComponent/aImageStorageComponent/dUpdateComponent";
...
const ImageStorageUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = imageStorageAPIEndpoint.useImageStorageRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = imageStorageAPIEndpoint.useImageStorageUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.imageStorage, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* ImageStorageUpdatePage */}
...
export default ImageStorageUpdatePage;
