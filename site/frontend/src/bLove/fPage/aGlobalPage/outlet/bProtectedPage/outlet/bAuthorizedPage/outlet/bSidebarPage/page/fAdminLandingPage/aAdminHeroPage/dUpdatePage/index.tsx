...
import AdminHeroUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/fAdminLandingComponent/aAdminHeroComponent/dUpdateComponent";
...
const AdminHeroUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = adminHeroAPIEndpoint.useAdminHeroRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = adminHeroAPIEndpoint.useAdminHeroUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.adminHero, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AdminHeroUpdatePage */}
