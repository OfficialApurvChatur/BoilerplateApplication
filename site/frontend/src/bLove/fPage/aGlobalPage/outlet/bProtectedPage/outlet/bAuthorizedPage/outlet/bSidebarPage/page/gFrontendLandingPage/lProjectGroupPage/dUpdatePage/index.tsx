...
import ProjectGroupUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/gFrontendLandingComponent/lProjectGroupComponent/dUpdateComponent";
...
const ProjectGroupUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = projectGroupAPIEndpoint.useProjectGroupRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = projectGroupAPIEndpoint.useProjectGroupUpdateAPIMutation();
  ...
  useSocketEventHook(socket, {
    [`PROJECTGROUP_RETRIEVED:${id}`]: () => apiCall.retrieveAPIResponse.refetch()
  })  
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.projectGroup, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* ProjectGroupUpdatePage */}
      <ProjectGroupUpdateComponent 
