...
import AdminProjectSectionUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/fAdminLandingComponent/kAdminProjectSectionComponent/dUpdateComponent";
...
const AdminProjectSectionUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = adminProjectSectionAPIEndpoint.useAdminProjectSectionRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = adminProjectSectionAPIEndpoint.useAdminProjectSectionUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.adminProjectSection, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AdminProjectSectionUpdatePage */}
      <AdminProjectSectionUpdateComponent 
...
export default AdminProjectSectionUpdatePage;
