...
import ProjectSectionUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/gFrontendLandingComponent/kProjectSectionComponent/dUpdateComponent";
...
const ProjectSectionUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = projectSectionAPIEndpoint.useProjectSectionRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = projectSectionAPIEndpoint.useProjectSectionUpdateAPIMutation();
  ...
  useSocketEventHook(socket, {
    [`PROJECTSECTION_RETRIEVED:${id}`]: () => apiCall.retrieveAPIResponse.refetch()
  })  
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.projectSection, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* ProjectSectionUpdatePage */}
