...
import SignInUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/cUserAuthenticationComponent/aSignInComponent/dUpdateComponent";
...
const SignInUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = signInAPIEndpoint.useSignInRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = signInAPIEndpoint.useSignInUpdateAPIMutation();
  ...
  useSocketEventHook(socket, {
    [`SIGNIN_RETRIEVED:${id}`]: () => apiCall.retrieveAPIResponse.refetch()
  })  
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.signIn, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* SignInUpdatePage */}
...
export default SignInUpdatePage;
