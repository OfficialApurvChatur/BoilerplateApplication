...
import AdminBranchUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/fAdminLandingComponent/jAdminBranchComponent/dUpdateComponent";
...
const AdminBranchUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = adminBranchAPIEndpoint.useAdminBranchRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = adminBranchAPIEndpoint.useAdminBranchUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.adminBranch, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AdminBranchUpdatePage */}
...
export default AdminBranchUpdatePage;
