...
import ContactInfoUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/gFrontendLandingComponent/eContactInfoComponent/dUpdateComponent";
...
const ContactInfoUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = contactInfoAPIEndpoint.useContactInfoRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = contactInfoAPIEndpoint.useContactInfoUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.contactInfo, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* ContactInfoUpdatePage */}
...
export default ContactInfoUpdatePage;
