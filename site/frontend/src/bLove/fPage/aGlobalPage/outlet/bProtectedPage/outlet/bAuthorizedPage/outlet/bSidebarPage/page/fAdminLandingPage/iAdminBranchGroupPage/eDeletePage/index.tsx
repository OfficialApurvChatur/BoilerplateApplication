...
import AdminBranchGroupDeleteComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/fAdminLandingComponent/iAdminBranchGroupComponent/eDeleteComponent";
...
const AdminBranchGroupDeletePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = adminBranchGroupAPIEndpoint.useAdminBranchGroupRetrieveAPIQuery({ params: { _id: id } });
  const [ deleteAPITrigger, deleteAPIResponse ] = adminBranchGroupAPIEndpoint.useAdminBranchGroupDeleteAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.adminBranchGroup, "Delete") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AdminBranchGroupDeletePage */}
...
export default AdminBranchGroupDeletePage;
