...
import SocialMediaContentDeleteComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/eContentOperationComponent/bSocialMediaContentComponent/eDeleteComponent";
...
const SocialMediaContentDeletePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = socialMediaContentAPIEndpoint.useSocialMediaContentRetrieveAPIQuery({ params: { _id: id } });
  const [ deleteAPITrigger, deleteAPIResponse ] = socialMediaContentAPIEndpoint.useSocialMediaContentDeleteAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.socialMediaContent, "Delete") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* SocialMediaContentDeletePage */}
...
export default SocialMediaContentDeletePage;
