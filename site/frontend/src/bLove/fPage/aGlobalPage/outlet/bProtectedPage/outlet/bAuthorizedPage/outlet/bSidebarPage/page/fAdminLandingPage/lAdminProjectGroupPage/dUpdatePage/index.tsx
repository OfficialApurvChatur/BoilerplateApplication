...
import AdminProjectGroupUpdateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/fAdminLandingComponent/lAdminProjectGroupComponent/dUpdateComponent";
...
const AdminProjectGroupUpdatePage = () => {
  // Variable
  const { id } = useParams();
  const socket = useSocket();
  const navigate = useNavigate();
  const retrieveAPIResponse = adminProjectGroupAPIEndpoint.useAdminProjectGroupRetrieveAPIQuery({ params: { _id: id } });
  const [ updateAPITrigger, updateAPIResponse ] = adminProjectGroupAPIEndpoint.useAdminProjectGroupUpdateAPIMutation();
  ...
  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.adminProjectGroup, "Update") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* AdminProjectGroupUpdatePage */}
...
export default AdminProjectGroupUpdatePage;
